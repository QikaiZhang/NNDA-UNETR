...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
    ...
    if task=='Task001_ACDC':
        ...
        plans['plans_per_stage'][Stage]['pool_op_kernel_sizes']=[[1,4,4],[2,2,2],[2,2,2]]
        # plans['plans_per_stage'][Stage]['pool_op_kernel_sizes']=[[1,5,5],[2,2,2],[2,2,2]]
        ...
        plans['plans_per_stage'][Stage]['patch_size']=np.array([96,96,96])
        plans['plans_per_stage'][Stage]['pool_op_kernel_sizes']=[[3,3,3],[2,2,2],[2,2,2]] # for deep supervision
        # plans['plans_per_stage'][Stage]['patch_size']=np.array([64,128,128])
        # plans['plans_per_stage'][Stage]['pool_op_kernel_sizes']=[[2,4,4],[2,2,2],[2,2,2]] # for deep supervision
        pickle_file = open(plans_file,'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
    ...
    elif task=='Task004_NIHPancreas':
        plans['plans_per_stage'][Stage]['batch_size']=3
        plans['plans_per_stage'][Stage]['patch_size']=np.array([96,96,96])
        plans['plans_per_stage'][Stage]['pool_op_kernel_sizes']=[[3,3,3],[2,2,2],[2,2,2]]
        pickle_file = open(plans_file,'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()

    elif task=='Task005_Lung':
        plans['plans_per_stage'][Stage]['batch_size']=1
        plans['plans_per_stage'][Stage]['patch_size']=np.array([32,192,192])
        plans['plans_per_stage'][Stage]['pool_op_kernel_sizes']=[[1,4,4],[2,2,2],[2,2,2]]
        pickle_file = open(plans_file,'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
