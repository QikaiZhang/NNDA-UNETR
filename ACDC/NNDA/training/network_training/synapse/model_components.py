...
from nnformer.training.network_training.synapse.transformerblock import TransformerBlock
        ...
        if conv_decoder == True:
            ...
            for j in range(att_depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size=out_channels, proj_size=proj_size,
                                                     num_heads=num_heads,
                                                     dropout_rate=0.15, pos_embed=True,att_head_dim=att_head_dim, windowsize = att_windowsize))
            self.decoder_block.append(nn.Sequential(*stage_blocks))
            # stage_blocks = []
