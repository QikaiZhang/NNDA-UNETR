...
def test(fold):
    ...
    path1 = '../DATASET/nnFormer_trained_models/nnFormer/3d_fullres/Task001_ACDC/nnFormerTrainerV2_nnformer_acdc__nnFormerPlansv2.1/'

    label_list=sorted(glob.glob(os.path.join(path,'labelsTs','*nii.gz')))
    # infer_list=sorted(glob.glob(os.path.join(path1,'inferTs',fold,'*nii.gz')))
    infer_list=sorted(glob.glob(os.path.join(path1,'inferTs','*nii.gz')))
