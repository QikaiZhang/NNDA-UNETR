...
from synapse.nnda_unetr_synapse import NNDA_UNETR 
import torch
import time
...
def main():
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            torch.cuda.synchronize()
            time_end = time.time()
            total_time.append(round((time_end-time_start) * 1000))
            ...
            for i in range(1, 14):
                ...
                organ_Dice_sum[i-1] += organ_Dice

            print('All dsc',dice_list_sub)
        ...
        organ_Dice_sum = [ x/len(val_loader) for x in organ_Dice_sum]
        print("Overall SEP_Mean Dice: {}".format(organ_Dice_sum))


        print("Inference Time(ms)：", sum(total_time)/len(total_time))
